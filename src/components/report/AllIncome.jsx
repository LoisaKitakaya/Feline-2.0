...
import { GET_ALL_INCOME_STATEMENTS } from "../../schema";
...
const AllIncome = ({ account_id }) => {
  const dispatch = useDispatch();

  const { data, error, loading } = useQuery(GET_ALL_INCOME_STATEMENTS, {
  ...
  return (
    <>
      <h1 className="text-xl mb-4 underline italic">Income Report History</h1>
      {data.getAllIncomeStatements.length !== 0 ? (
        <>
          {data.getAllIncomeStatements.map((report, index) => {
            const incomeElement = (
              <Link to={`/income/${report.uid}`} key={report.id}>
        ...
        <>
          <div className="my-20 text-center">
            <h4 className="font-semibold text-2xl">
              You have not yet generated any income reports.
            </h4>
          </div>
        </>
      )}
